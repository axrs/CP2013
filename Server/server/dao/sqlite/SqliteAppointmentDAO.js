...
var SqliteAppointmentDAO = Ring.create([SqliteHelper, IAppointmentDAO], {
    ...
    retrieveAll: function (callback) {
        var sql = 'SELECT AppointmentId, Appointment.TypeId, Appointment.ProviderId, ' +
            ...
            'LEFT JOIN Provider on Provider.ProviderId = Appointment.ProviderId ' +
            'LEFT JOIN Contact on Contact.ContactId = Appointment.ContactId ' +
            'LEFT JOIN Appointment_Type on Appointment_Type.TypeId = Appointment.TypeId;';
        this.all(sql, null, function (err, result) {
            var appointments = [];
    ...
    retrieveById: function (id, callback) {
        var sql = 'SELECT AppointmentId, Appointment.TypeId, Appointment.ProviderId, ' +
            ...
            'LEFT JOIN Contact on Contact.ContactId = Appointment.ContactId ' +
            'LEFT JOIN Appointment_Type on Appointment_Type.TypeId = Appointment.TypeId ' +
            'WHERE Appointment.AppointmentId=$id LIMIT 1;';
    ...
    retrieveForProvider: function (providerId, callback) {
        var sql = 'SELECT AppointmentId, Appointment.TypeId, Appointment.ProviderId, ' +
            ...
            'LEFT JOIN Contact on Contact.ContactId = Appointment.ContactId ' +
            'LEFT JOIN Appointment_Type on Appointment_Type.TypeId = Appointment.TypeId ' +
            'WHERE Appointment.ProviderId=$id;';
    ...
    retrieveForContact: function (contactID, callback) {
        var sql = 'SELECT AppointmentId, Appointment.TypeId, Appointment.ProviderId, ' +
            ...
            'LEFT JOIN Contact on Contact.ContactId = Appointment.ContactId ' +
            'LEFT JOIN Appointment_Type on Appointment_Type.TypeId = Appointment.TypeId ' +
            'WHERE Appointment.ContactId=$id;';
